...
export class CurrencyTextField extends React.Component<
  ...
  any
> {
  autonumeric: AutoNumeric | undefined = undefined;
  input: string | HTMLInputElement | HTMLElement | undefined = undefined;
  ...
  componentWillReceiveProps(newProps: CurrencyTextFieldProps) {
    const isValueChanged =
      this.props.value !== newProps.value && this.getValue() !== newProps.value;

    if (isValueChanged) {
      this.autonumeric?.set(
        newProps.value === undefined ? null : newProps.value
      );
    }
  }
  ...
  getValue() {
    ...
    return valueMapper[this.props.outputFormat || 'number'](this.autonumeric);
  }
  callEventHandler(
