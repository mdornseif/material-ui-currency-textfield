...
```html
outputFormat='number' (default) | 'string'
```
...
minimumValue={-10000000000000} (default) | "Value can be increased and
decreased"
```
...
'@unicef/material-ui-currency-textfield' export default function MyComponent() {
const [value, setValue] = React.useState(100); return ( <CurrencyTextField
label="Amount" variant="outlined" value={value} currencySymbol="$"
//minimumValue="0" outputFormat="string" decimalCharacter="."
digitGroupSeparator="," //autoFocus //readonly //disabled
//placeholder="Currency" //preDefined={predefinedOptions.percentageEU2dec}
onChange={(event, value)=> setValue(value)} /> ); }
```
